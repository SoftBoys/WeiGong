...
- (instancetype)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        ...
        [self addSubview:self.labremindcontent];
        ...
        [self.labremind mas_makeConstraints:^(MASConstraintMaker *make) {
            make.top.mas_equalTo(self.button_auth.mas_bottom).offset(8);
            make.left.mas_equalTo(remindX);
        }];
        
        CGFloat remindW = [self.labremind systemLayoutSizeFittingSize:UILayoutFittingCompressedSize].width;
        CGFloat contentW = kScreenWidth- remindX*2 - remindW;
        [self.labremindcontent mas_makeConstraints:^(MASConstraintMaker *make) {
            make.left.mas_equalTo(self.labremind.mas_right);
            make.width.mas_equalTo(contentW);
            make.top.mas_equalTo(self.labremind);
        }];
        
        NSString *content = @"更换登陆账号后，新账号需要重新绑定微信才能收到推送信息";
        self.labremindcontent.text = content;
    }
    return self;
}
...
- (UILabel *)labremind {
    if (!_labremind) {
        _labremind = [UILabel wg_labelWithFont:kFont(12) textColor:kColor_Black_Sub];
        _labremind.text = @"提示：";
    }
    return _labremind;
}
- (UILabel *)labremindcontent {
    if (!_labremindcontent) {
        _labremindcontent = [UILabel wg_labelWithFont:self.labremind.font textColor:self.labremind.textColor];
        _labremindcontent.numberOfLines = 0;
    }
    return _labremindcontent;
}
#pragma mark - 验证
- (void)authData {
