...
+ (NSString *)wg_platformString {
    NSString *platform = [self wg_platform];
...
+ (NSUInteger)wg_getSysInfo:(uint)typeSpecifier {
...
+ (NSUInteger)wg_totalMemoryBytes {
    return [self wg_getSysInfo:HW_PHYSMEM];
}
...
@implementation UIDevice (Camara_Photo)

+ (BOOL)wg_isAccessCamera {
    // 判断是否取得相机授权
    AVAuthorizationStatus status = [AVCaptureDevice authorizationStatusForMediaType:AVMediaTypeVideo];
    
    // Restricted:受限 Denied:拒绝授权
...
+ (void)wg_showCameraAlert {
    ...
    NSString *title = @"温馨提示";
    NSString *message = [NSString stringWithFormat:@"请在设备的\"设置-隐私-照片\"选项中，允许%@访问你的相机",name];
    NSString *cancel = @"取消", *sure = @"设置";
    
    [UIAlertController wg_alertWithTitle:title message:message completion:^(UIAlertController *alert, NSInteger index) {
        if (index) {
            NSString *boundleId = [[NSBundle mainBundle] infoDictionary][@"CFBundleIdentifier"];
